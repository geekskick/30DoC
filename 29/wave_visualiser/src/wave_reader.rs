...
fn get_term_size() -> (u32, u32) {
    let raw_term = term_size::dimensions().unwrap_or((200, 50));
    // textplots crate needs at least 32x32 otherwise it panics
    let resized = (
        std::cmp::max((raw_term.0 * 2) - 1, 32),
        std::cmp::max((raw_term.1 * 2) - 1, 32),
    );
    (resized.0 as u32, resized.1 as u32)
}

fn plot_part(f: &mut fmt::Formatter, number: usize, of: usize, data: &[(f32, f32)]) -> fmt::Result {
    let term = get_term_size();
    let chunk_size = data.len() / of;
    let start = number * chunk_size;
    let end = ((number + 1) * chunk_size) - 1;

    writeln!(f, "Part {} of {}", number + 1, of)
        .expect("Unable to write graph part title to the formatter");
    writeln!(
        f,
        "{}",
        Chart::new(term.0, term.1, start as f32, end as f32)
...
impl fmt::Display for WaveFile {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        writeln!(f, "{}", self.header).expect("Error writing to the formatter");
        let plot_points: Vec<(f32, f32)> = self
            .ldata
            .iter()
            .enumerate()
            .map(|(idx, sample)| (idx as f32, *sample as f32))
            .collect();

        plot_part(f, 0, 4, &plot_points).expect("Unable to plot first part of samples");
        plot_part(f, 1, 4, &plot_points).expect("Unable to plot first part of samples");
        plot_part(f, 2, 4, &plot_points).expect("Unable to plot third part of samples");
        plot_part(f, 3, 4, &plot_points)
    }
}
